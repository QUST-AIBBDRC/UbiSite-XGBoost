...
yscore_sum.to_csv('yscore_sum_ET_lassoset1A1_feature0.005.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('ytest_sum_ET_lassoset1A1_feature0.005.csv')
fpr, tpr, _ = roc_curve(ytest[:,0], yscore[:,0])
auc_score=np.mean(scores, axis=0)[7]
lw=2
plt.plot(fpr, tpr, color='darkorange',
lw=lw, label='ET ROC (area = %0.2f%%)' % auc_score)
...
data_csv.to_csv('ET_lassoset1A1_feature0.005.csv')
